...
分别将ESP-01S与UNO开发板的代码上传成功后，将ESP-01S连接到UART。按一下“ESP-01S Arduino wifi转串口扩展板”上的`RST`按键使ESP-01S模块复位重新连接WiFi并通过UNO开发板的串口打印IP地址，然后再连接同一个wifi设备的浏览器中输入IP扫搜进入网页控制页面。
