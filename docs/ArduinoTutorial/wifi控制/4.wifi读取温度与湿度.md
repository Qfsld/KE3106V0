...
## 2.2.5.4 ESP01-S 代码说明

打开代码`ESP01_S_Code`代码文件，将不用显示的按键以及传感器数据显示注释掉，将温度与湿度数据显示取消注释即可（你也可以使用快捷键`Ctrl + /`，鼠标光板点击到需要注释的代码行或选中的代码多行代码按`Ctrl + /`即可达到注释或者取消注释的效果 ）
...
② 使用`if`判断是否接收到ESP01-S模块发送的字符`SENSOR_READ`，如果是就使用串口打印函数发送数据给ESP01-S模块

③ 发送方式假如需要发送温度值25给ESP01-S模块显示到网页上发送格式如下:`Serial.println("TEMP" + String(25))`，湿度也是以同样方式发送的
