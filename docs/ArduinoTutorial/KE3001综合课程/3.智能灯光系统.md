...
如果你不想起床关灯开灯时你是不是再想喊一声通过语音模块或者通过手机控制卧室灯，客厅灯，厨房灯，过道灯呢？本次课程就是教你如何使用ESP01-S模块加语音模块控制LED灯模拟家庭中你所需要开关的灯。
...
③ 读取模拟串口中语言模块发送的控制指令，并赋值给变量`Voice_Control`； 读取串口中ESP01-S模块发送的控制指令，并赋值给变量`WiFi_Control`
