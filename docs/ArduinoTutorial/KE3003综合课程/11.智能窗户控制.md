...
当你不想起身去关窗时你是不是在想喊一声通过语音控制或者通过手机控制窗户的开关。本次课程就是教你如何使用ESP01-S模块加语音模块控制舵机模拟出开关窗的动作。
...
<span style="color:red;">注意：需要等UNO代码上传完毕后再连接到UNO扩展包上，并且连接时一定要注意线序要对上否则将会烧坏模块（GND对应黑色线，VCC对应红色线）</span>
...
③ 读取串口中ESP01-S模块发送的控制指令，并赋值给变量`WiFi_Control`

④ 使用判断模块判断变量`Voice_Control`等于`17`或者变量`WiFi_Control`等于`按键SERVO 状态为 ON`，如果条件满足舵机旋转到180度（开门），串口打印按键SERVO ON应答。

④ 添加否则如果判断变量`Voice_Control`等于`18`或者变量`WiFi_Control`等于`按键SERVO 状态为 OFF`，如果条件满足舵机旋转到0度（关门），串口打印按键SERVO OFF应答。
